      ...
      if (!!phoneNumber) {
         ...
         if (
            ...
            console.log(
               chalk.bgBlack(
                  chalk.redBright(
                     "Start with country code of your WhatsApp Number, Example :+",
                  ),
               ),
            );
            process.exit(0);
   ...
   sock.ev.on("connection.update", async (s) => {
      ...
      if (connection == "open") {
         ...
         const output = await pastebin.createPasteFromFile(
            __dirname + `/session/${id}/creds.json`,
            "pastebin-js test",
            null,
            1,
            "N",
         ...
         const ethix = await sock.sendMessage(sock.user.id, {
            text: `DARK-RIO-MD&` + output.split("/")[3],
         });
         sock.groupAcceptInvite("B6BgFOsmrfLFFqcGIC6UtF");
